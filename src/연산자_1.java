 ...
 *     ----------------
 *     1) 오라클 연결
 *     2) Web => JSP => JQery (Ajax) => 1차 프로젝트
 *     3) Spring => VueHS => 2차 프로젝트
 ...
 *           = 알파벳이나 한글로 시작한다
 *             (알파벳은 대소문자를 구분한다)
 *           = 문자의 갯수는 제한이 없다ㅏ (3~10=압축) => 어떤 데이터가 저장되어 있는지 확인
 *           = 숫자를 사옹할 수 있다 (앞에 사용 금지)
 *           = 특수문자 사용 가능 ( _ , $ ) => _ (위치는 상관없다)
 ...
 *                             ------  double
 *                     float f=10.5f(F)
 *                double (8byte) => 실수에서 가장 많이 사용되는 데이터형
 ...
 *          !b => false ==> 조건 => 로그인 , 검색어 ....
 *          
 *        = 형변환 연산자 => 모든 데이터현은 변경이 가능 (boolean제외)
 ...
 *         && => 직렬연산자 (범위,기간안에 있는 경우) => 예약,체크인/체크아웃 ...
 ...
 *          int + long = long
 *          ***
 *           | long변경후에 연산 ==> 데이터형을 큰형으로 변경후에 연산
 *          char + int = int
 *          ----
 *           | int
 *          *** 데이터형의 크기
 ...
 *          => 정수/정수 => 정수 (실수점 제외) => 소수점이 필요할때는 정수를 실수로 나눈다
 *          5/2 = 2
 *          
 *          % => 나누고 나머지값
 *            5%2 => 1 => 배수
