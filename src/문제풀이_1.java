 ...
 *           double d=10;
 *           ------   --
 *           double   int ==> 10은 10.0으로 변경해서 저장 ==> 자동형변환
 *        3. 변수명 만들기
 *           1) 알파벳,한글로 시작한다
 *              int abc; => int Abc; (대소문자 구분)
 *              int 국어점수;
 *           2) 숫자사용이 가능
 *              int a1a,aaaaa1,baba12bad212... (맨 앞에는 사용 금지)
 *           3) 특수문자 ( _ , $ ) => 위치는 상관없다
 *              int _temp int file_size... $는 사용빈도가 거의 없다
 *           4) 키워드 : 이미 사용중인 자바 단어
 *                     int , long...
 *           5) 공백 사용 금지
 *           6) 변수의 길이는 제한이 없다 (3~10) 압축하는게 국룰
 *        4. 데이터형
